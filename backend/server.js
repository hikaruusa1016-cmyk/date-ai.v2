...
async function generateMockPlan(conditions, adjustment, allowExternalApi = true) {
  ...
  if (allowExternalApi && hasPlacesAPI && (!lunchPlace || !activityPlace || !cafePlace || !dinnerPlace)) {
    ...
    // 予算レベルに応じた検索キーワード
    // 予算レベルに応じた検索キーワード（デート特化版）
    const lunchKeywords = {
      // 脱・日常食。カフェ飯や専門店など「安くてもおしゃれ」なラインナップ
      low: ['おしゃれカフェランチ', 'グルメバーガー人気', 'ベーカリーカフェ イートイン', 'ガレットランチ', 'エスニックランチ おしゃれ', 'サンドイッチ専門店'],
      // 少し背伸びしたビストロや、きちんとした和食
      medium: ['隠れ家ランチ', 'ビストロランチ', 'トラットリア人気', 'モダン和食ランチ', '洋食屋おすすめ', 'シーフードランチ'],
      // 記念日や特別な日に
      high: ['ホテルランチ', 'プリフィクスコース', '老舗洋食', 'アフタヌーンティー', '寿司ランチ カウンター'],
    };
    const dinnerKeywords = {
      // 居酒屋排除。バルや専門料理で「あえてのカジュアル」を演出
      low: ['ワインバル カジュアル', 'ネオ居酒屋 おしゃれ', 'ピッツェリア人気', 'タコス専門店', 'クラフトビール専門店', 'スパニッシュバル'],
      // 雰囲気重視。
      medium: ['隠れ家レストラン', 'オステリア人気', 'ビストロおすすめ', '創作和食デート', '薪焼き料理', 'ダイニングバー雰囲気'],
      // 体験重視。
